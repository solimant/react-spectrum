...
interface AvatarAria {
  ...
  /**
   * Whether the avatar is currently pressed.
   */
  isPressed: boolean
}
 ...
 */
export function useAvatar(props: AriaAvatarOptions, ref: RefObject<HTMLElement>): AvatarAria {
  const {
    elementType = 'img',
    alt = '',
    isDisabled,
  ...
  const {isPressed, pressProps} = usePress({onPress, onPressStart, onPressEnd, isDisabled, ref});

  return {
    isPressed,
    avatarProps: mergeProps(domProps,
      {
        ...pressProps,
        ...avatarProps,
